...
# Trigger full harvest (~500K trials, takes ~10-15 min)
curl -X POST "http://localhost:8000/harvest/trigger?full=true"

# Monitor progress
curl http://localhost:8000/harvest/status

# Or incremental harvest (new/updated trials since a date)
curl -X POST "http://localhost:8000/harvest/trigger?since=2026-03-29"
```
...
# {"total_trials": 502341, "last_harvest": "2026-03-31T...", "status": "idle"}
```
...
```bash
curl "http://localhost:8000/trials?limit=10&status=RECRUITING"
```

### `GET /trials/{registry_id}`
Single trial by registry ID (e.g., NCT number).
```bash
curl http://localhost:8000/trials/NCT07226206
```
...
```bash
# CSV
curl -o trials.csv http://localhost:8000/trials/bulk

# NDJSON
curl -o trials.ndjson "http://localhost:8000/trials/bulk?format=ndjson"
```
...
```bash
# First call
curl "http://localhost:8000/trials/updates?since=2026-03-30"
# Response includes: "cursor": "2026-04-01T00:00:00"

# Next call — use the cursor
curl "http://localhost:8000/trials/updates?since=2026-04-01T00:00:00"
```
...
```bash
# On your server
git clone <repo-url> && cd clinicaltrial
python3 -m venv .venv && source .venv/bin/activate
pip install -r requirements.txt
uvicorn app.main:app --host 0.0.0.0 --port 8000
```
